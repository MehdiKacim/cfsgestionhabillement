...
namespace cfsgestionhabillement.Repository
{
    class UserRepository
    {
        ...
        public User CheckConnexion(string username, string password)
        {
            User user = new User();
            Fonction fonction = new Fonction();
            string hash;
            ...
            MySqlConn.Open();
            var cmd = new MySqlCommand("SELECT * FROM secouristes,fonctions where USERNAME='" + username + "' AND PASSWORD='" + hash + "' AND secouristes.ID_FONCTION_ENUM = fonctions.ID_FONCTIONS;", MySqlConn);
